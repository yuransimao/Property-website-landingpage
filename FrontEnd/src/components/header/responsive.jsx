...
import { RxCross2 } from "react-icons/rx";
 ...
 const ResponsiveMenu = ({setVisivelMenu,HandleSign}) => {
         ...
         <div className='bg-zinc-900/20  fixed top-0 bottom-0 w-full z-[60] left-0 right-0'>
          ...
          <div className='absolute flex flex-col justify-between bg-white right-0 w-11/12 md:w-3/6 sm:w-[70%] h-dvh p-10 rounded-l'>
            ...
            <ul className='flex  flex-col gap-9 py-4' onClick={() =>setVisivelMenu(false)}>
                ...
                <button className='bg-blue-500 py-2 px-5 rounded text-white font-medium transition-all hover:opacity-[0.8]' onClick={ () => HandleSign()}>Sign in</button>
            </div>
            </div>
             </div>
             </div>
